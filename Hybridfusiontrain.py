...
def DNN_early():
    train_input = Input(shape=(vector_size * 2 * 3,), name='Inputlayer')
...
def feature_vector(feature_name, df, vector_size):
    ...
    print('allfeature number',len(all_feature))
...
def cross_validation(feature_matrix, label_matrix, clf_type, event_num, seed, CV, set_name):
    ...
    for k in range(CV):
        ...
        pred_d = np.zeros((len(test_index[0]), event_num), dtype=float)
        #dnn=DNN()


        for i in range(len(feature_matrix)-2):
            x_train = feature_matrix[i][train_index]
            x_test = feature_matrix[i][test_index]
            for j in range(1,2):
                x_train  = np.column_stack((x_train,feature_matrix[j][train_index]))
                x_test = np.column_stack((x_test, feature_matrix[j][test_index]))
        ...
        for i in range(len(feature_matrix)):
            ...
            if clf_type == 'MDDI':
                ...
                clf = LogisticRegression()
            clf.fit(x_train, y_train)
            pred_d += clf.predict_proba(x_test)
            
        #dnn.save('target_model_weight_{}.h5'.format(k))
        pred_score = (pred + pred_d/ len(feature_matrix))/2
        pred_type = np.argmax(pred_score, axis=1)
        result_all, result_eve = evaluate(pred_type, pred_score, y_test, event_num, set_name)
        print(result_all)
...
def evaluate(pred_type, pred_score, y_test, event_num, set_name):
    ...
    #print( "average_precision_score micro",average_precision_score(y_one_hot, pred_score, average='micro'))
    #print( "average_precision_score macro",average_precision_score(y_one_hot, pred_score, average='macro'))
...
def main(args):
    seed = 0
    CV = 5
    df_drug =  pd.read_csv('/home/huangan/workspace/TMDDIE/dataset/drug_information_1258_519.csv')

    feature_list = args['featureList']
    feature_list = ['target','enzyme']
    ...
    #     # =============================================================================
         save_result(featureName, 'all519', clf, all_result)
         save_result(featureName, 'each519', clf, each_result)
...
if __name__ == "__main__":  
    ...
    parser.add_argument("-c","--classifier",choices=["MDDI","RF","KNN","LR"],default=["MDDI"],help="classifiers to use",nargs="+")
